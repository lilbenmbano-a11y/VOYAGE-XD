...
async function handleDeleted(update) {
  ...
  if (!id) return
  // Baileys emits a REVOKE update with messageStubType. Keep this tolerant across versions.
...
async function processCommand(m) {
  ...
  // Multi-user provisioning: a person may request a session for their own number.
  // The master owner may also provision a user. A pairing request never grants
  // control of an existing session to another WhatsApp account.
  if (cmd === 'pair') {
  ...
  if (cmd === 'sessions') {
    ...
    return send(jid,{text:`📱 *SCOTTY SESSIONS*\n\n${list}`})
  }
  if (cmd === 'logout') {
    if (!isOwner(m.key.participant||jid)) return send(jid,{text:'❌ Owner only.'})
    ...
    return send(jid,{text:'✅ Your SCOTTY session has been logged out. Pair again when needed.'})
  }
  ...
  try {
    ...
    if (cmd==='ai') {
      ...
      return send(jid,{text:`🤖 *SCOTTY AI*\n\n${await ai(argText)}`})
    }
    ...
    // aliases and settings shown in the SCOTTY command reference
    ...
    if (cmd==='description') { if(args[0]) { if(!isOwner(m.key.participant||jid)) return send(jid,{text:'❌ Owner only.'}); setSetting('description',argText); return send(jid,{text:'✅ Description updated.'}) } return send(jid,{text:`📝 ${settings.description||'SCOTTY BOT'}`}) }
    if (cmd==='stickername') { if(args[0]) { if(!isOwner(m.key.participant||jid)) return send(jid,{text:'❌ Owner only.'}); setSetting('stickerName',argText); return send(jid,{text:'✅ Sticker pack name updated.'}) } return send(jid,{text:`🏷️ ${settings.stickerName||'SCOTTY BOT'}`}) }
    ...
    if (cmd==='settings') return send(jid,{text:`⚙️ *SCOTTY SETTINGS*\n\n${JSON.stringify(settings,null,2).slice(0,6000)}`})
    ...
    if (cmd==='botdp') { try { const url=await sock.profilePictureUrl(jidNormalizedUser(sock.user.id),'image'); return send(jid,{image:{url},caption:'🤖 SCOTTY BOT DP'}) } catch { return send(jid,{text:'❌ Bot profile picture unavailable.'}) } }
    ...
    if (cmd==='newgc') { if(!isPrivileged(m.key.participant||jid)) return send(jid,{text:'❌ Owner/sudo only.'}); const name=argText||'SCOTTY GROUP'; const t=m.key.participant||jid; const created=await sock.groupCreate(name,[t]); return send(jid,{text:`✅ Group created: ${created?.id||'created'}\nUse ${getPrefix()}link inside the new group.`}) }
    ...
    if (cmd==='ik') return send(jid,{text:'⚡ SCOTTY BOT is active and ready.'})
    if (cmd==='pair') return send(jid,{text:'📱 Pairing is configured at startup with PAIRING_NUMBER. Restart the Pterodactyl server after setting it in .env.'})
...
async function autoReply(m) {
  ...
  try {
    const answer=await ai(`You are SCOTTY BOT, a friendly WhatsApp assistant. Reply briefly and naturally to this message: ${text}`)
    ...
    await runInSession(session, async()=>{
      if(qr && session.pairingNumber && !state.creds.registered && !pairingCodeRequested) {
        ...
        try {
          ...
          console.log(`\\n[SCOTTY] Pairing ${session.number}: ${formatted}\\n`)
          ...
          console.error(`[SCOTTY] Pairing-code request failed for ${session.number}:`,e?.message || e)
          if(session.pairController?.sock && session.pairRequester) {
      ...
      if(connection==='open') {
        ...
        session.paired=true
        saveSessionRegistry()
        console.log(`⚡ SCOTTY session connected: ${session.number}`)
        console.log(`📌 Prefix: ${session.settings.prefix || config.prefix}`)
        if(controller?.sock && requester) await controller.sock.sendMessage(requester,{text:'✅ SCOTTY BOT paired successfully. Your personal bot is now online. Your settings and data are isolated from other users.'}).catch(()=>{})
...
async function bootAll() {
  ...
  // Keep the original single-user paths for the master bot so upgrading does not
  // force the existing SCOTTY account to pair again.
  ...
  // Persist existing user sessions and start them. Do not trust registry auth paths.
