...
app.get('/status', (req, res) => {
  const sessionList = [...sessions.values()].map(s => ({
    ...
    number: s.number,
    paired: s.paired,
    uptime: s.startedAt ? formatUptime((Date.now() - s.startedAt) / 1000) : 'N/A'
  }));
  res.json({
    ...
    onlineSessions: sessionList.filter(s => s.paired).length
  });
});
...
app.listen(PORT, () => {
  ...
  console.log(`⚡  Starting VOYAGE-MD bot sessions...\n`);
...
const settings = new Proxy({}, {
  get(_t, p) { return currentSession().settings[p]; },
  set(_t, p, v) { currentSession().settings[p] = v; return true; }
});

const sock = new Proxy({}, {
  get(_t, p) {
    const v = currentSession().sock?.[p];
    return typeof v === 'function' ? v.bind(currentSession().sock) : v;
  }
});

const messageCache = new Proxy({}, {
  get(_t, p) {
    const v = currentSession().messageCache[p];
    return typeof v === 'function' ? v.bind(currentSession().messageCache) : v;
  }
});

const groupCache = new Proxy({}, {
  get(_t, p) {
    const v = currentSession().groupCache[p];
    return typeof v === 'function' ? v.bind(currentSession().groupCache) : v;
  }
});
...
async function processCommand(m) {
  ...
  try {
    ...
    if (cmd === 'owner') return await send(jid, { text: ownerCard });
...
// ═══════════════════════════════════════════════════════════
//  SESSION STARTER (with web pairing hooks)
// ═══════════════════════════════════════════════════════════
async function startSession(session) {
    ...
    await runInSession(session, async () => {
      if (qr && session.pairingNumber && !state.creds.registered && !pairingCodeRequested) {
        ...
        try {
          ...
          // ─── RESOLVE WEB DASHBOARD REQUEST ───
          const pending = pendingPairings.get(session.pairingNumber);
          if (pending) {
            pending.resolve({ success: true, code: formatted });
            clearTimeout(pending.timeout);
            pendingPairings.delete(session.pairingNumber);
          }
          // ─────────────────────────────────────
          ...
          // ─── REJECT WEB DASHBOARD REQUEST ───
          const pending = pendingPairings.get(session.pairingNumber);
          if (pending) {
            pending.reject(new Error(e?.message || 'Pairing code generation failed'));
            clearTimeout(pending.timeout);
            pendingPairings.delete(session.pairingNumber);
          }
          // ─────────────────────────────────────
  ...
  s.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    await runInSession(session, async () => {
      for (const m of messages) await handleIncoming(m).catch(e => logger.error({ err: e }, 'message handler failed'));
    });
  });

  s.ev.on('messages.update', async updates => {
    await runInSession(session, async () => {
      for (const u of updates) await handleDeleted(u).catch(e => logger.warn({ err: e }, 'delete handler failed'));
    });
  });
